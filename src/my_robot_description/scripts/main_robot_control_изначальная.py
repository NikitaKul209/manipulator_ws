...
def train(model_name, algorithm,num_timesteps):
        ...
        try:
            if algorithm == 'DQN':
                model = DQN("MlpPolicy", env, device='cuda', verbose=1, learning_starts=500,gamma=0.95,tensorboard_log="src/my_robot_description/logs/logs_3d_manipulator/DQN")
            if algorithm =="PPO":
                model = PPO('MlpPolicy', env, device='cuda',verbose=1,gamma=0.8,tensorboard_log="src/my_robot_description/logs/logs_3d_manipulator/PPO")
