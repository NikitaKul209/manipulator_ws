...
class CustomEnv(gym.GoalEnv):
    ...
    def __init__(self, ):
        ...
        rospy.Subscriber('my_robot/joint_states', JointState, self.callback_joint_states,queue_size = 1, buff_size =2**24)
        # rospy.Rate(100)
        ...
        # self.action_space = gym.spaces.Box(low=np.float32(np.array([-3.14, -2.094, -2.094, -1.57])),
        #                                high=np.float32(np.array([3.14, 0, 0, 1.57])),
        #                                dtype=np.float32,
        #                                shape=(4,))

        self.observation_space = gym.spaces.Dict(dict(
            observation=gym.spaces.Box(low=np.float32(np.array([-3.14, -2.094, 2.094, -1.57, -3, -3, -3, -3, -3, -3])),
                                       high=np.float32(np.array([3.14, 0, 0, 1.57, 3, 3, 3, 3, 3, 3])),
                                       dtype=np.float32,
                                       shape=(10,)),
    ...
    def callback_joint_states(self, msg):
        self.angles = np.array(msg.position)
    ...
    def step(self, action):
        ...
        # self.angles[0] = np.clip(self.angles[0], np.radians(-180), np.radians(180))
        # self.angles[1:3] = np.clip(self.angles[1:3], np.radians(-120), np.radians(0))
        # self.angles[3] = np.clip(self.angles[3], np.radians(-90), np.radians(90))
        ...
        print("Итерация: {}\nТекущая длина эпизода: {}\nЦель: {}\nВыбрано действие: {}\nТекущий угол 1: {}\nТекущий угол 2: {}\nТекущий угол 3: {}\nТекущий угол 3: {}\nПозиция по Х: {}\nПозиция по Y: {}\nПозиция по Z: {}\nНаграда: {}\n".format(self.iteration,self.episode_length,self.goal,action,self.angles[0],self.angles[1],self.angles[2],self.angles[3], self.position[0],self.position[1],self.position[2],reward))
        # rospy.sleep(0.001)
    ...
    def reset(self):
        ...
        self.error = 0
        self.episode_length = 0
        a1 = np.radians(random.randrange(-180, 180, 5))
        a2 = np.radians(random.randrange(-120, 0, 5))
        a3 = np.radians(random.randrange(-120, 0, 5))
        a4 = np.radians(random.randrange(-90, 90, 5))
