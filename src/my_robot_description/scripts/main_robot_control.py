...
class CustomEnv(gym.GoalEnv):
    ...
    def step(self, action):
        ...
        self.angles[1:3] = np.clip(self.angles[1:4], -120, 0)
    ...
    def get_new_goal(self):
        x = random.choice([random.uniform(0.5, 1.2), random.uniform(-1.2, -0.5)])
        y = random.choice([random.uniform(0.5, 1.2), random.uniform(-1.2, -0.5)])
        z = random.uniform(1, 2.0)
        goal = np.array([x, y, z])
        return goal
